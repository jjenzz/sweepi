...
describe('no-componenttype-props', () => {
  ruleTester.run('no-componenttype-props', rule, {
    valid: [
      ...
      "import type { ComponentType } from 'react'; interface Config { renderer: ComponentType<{ q: string }>; }",
    ],
    invalid: [
      {
      ...
      {
        code: "import type { ComponentType } from 'react'; interface Props { Icon: IconAlias; } type IconAlias = ComponentType<{ size?: number }>;",
        errors: [
          {
            messageId: 'noComponentTypeProps',
            data: { prop: 'Icon' },
